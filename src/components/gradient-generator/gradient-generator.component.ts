...
import { AbsTemplate, AbsTemplatePrintMethod } from 'abs-template';
import { DEFAULT_FRAME, INIT_FRAME } from '../../general/consts';
import { Gradient, ProfilePictureFrame, Step } from '../../general/interfaces';
import { imageLoaded, activeFrame } from '../../general/observables';
...
export class GradientGeneratorComponent implements AbsComponent {
  ...
  private output: ProfilePictureFrame = INIT_FRAME;
  ...
  ready() {
    imageLoaded.pipe(skip(1)).subscribe(() => {
      this.onCanvasGenerated();
    });
  }
  ...
  addNewStep(stepData?: Step): HTMLElement {
    ...
    AbsTemplate.build({
      templateNode: this.stepTemplateNode as HTMLElement,
      printTargetNode: stepsListNode,
      printMethod: AbsTemplatePrintMethod.BEFORE_END,
    });
    const newStepNode = stepsListNode.querySelector(`${this.STEP_SELECTOR}:last-child`) as HTMLElement;
  ...
  generateDefaultGradient(): void {
    DEFAULT_FRAME.gradient?.steps?.forEach(step => {
